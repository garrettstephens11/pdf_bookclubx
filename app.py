...
# Initialize Flask app and configure server-side sessions
app = Flask(__name__)
...
def generate():
    ...
    use_xai = request.form.get('use_xai', 'false') == 'true'

    # Build the prompt based on your requirements
    prompt = f"""
    ...
    This discussion should involve at least two "turns" per discussion participant in this discussion. The discussion should address the book club question: 'What is a two-sentence quote from the text that stood out to you in reading this text? What does that quote bring to mind for you? Then, relate your thought back to the passage's message.'
    """

    if relation_text:
        prompt += f"\n\nNext, have the group participants relate the reading to {relation_text}."
    ...
    try:
        ...
        return jsonify({'error': 'Failed to generate discussion.'}), 500
    ...
    discussion_num = len(session['discussions'][str(index)]) - 1
...
def generate_audio():
    index = request.form.get('index')
    discussion_num = int(request.form.get('discussion_num'))
...
@app.route('/talk_to_segment', methods=['POST'])
def talk_to_segment():
    segment = request.form.get('segment', '')
    model = request.form.get('model', 'openai')
    user_query = request.form.get('query', '')

    if not segment or not user_query:
        return jsonify({'error': 'No segment or query provided'}), 400

    prompt = f"""You are a helpful assistant. The user has a segment of text:

    {segment}

    The user has asked a question about this segment:
    "{user_query}"

    Please provide a helpful answer based on the segment.
    """
    ...
    try:
        ...
        logging.error(f"Error during talk_to_segment API call: {e}")
        return jsonify({'error': 'Failed to talk to segment.'}), 500
